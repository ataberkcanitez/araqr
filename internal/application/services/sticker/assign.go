...
func (s *Service) Assign(ctx context.Context, req *web.AssignStickerRequest) (*sticker2.Sticker, error) {
	stx, err := s.stickerRepository.GetByID(ctx, req.StickerID)
	if err != nil {
	...
	assign, err := s.stickerRepository.Assign(ctx, stx)
	if err != nil {
		stx.UserID = ""
